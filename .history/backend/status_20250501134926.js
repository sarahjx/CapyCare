...
function decreaseHunger() {
  if (!currentPet || isGameOver || isPaused) return;
  // Food lovers get hungry faster, but not as fast as before
...
function decreaseEnergy() {
  if (!currentPet || isGameOver || isPaused) return;
  // Active pets lose energy faster, but not as fast as before
...
function decreaseHappiness() {
  if (!currentPet || isGameOver || isPaused) return;
  // Clingy pets lose happiness faster when not attended to, but not as fast as before
...
function updateIntervals() {
  ...
  // Set new intervals with much faster timing for testing
  hungerInterval = setInterval(decreaseHunger, 2000); // Every 2 seconds
  energyInterval = setInterval(decreaseEnergy, 2000); // Every 2 seconds
  hygieneInterval = setInterval(decreaseHygiene, 2000); // Every 2 seconds
  happinessInterval = setInterval(decreaseHappiness, 2000); // Every 2 seconds
}
